...
const columns: Column<PecaBrecho>[] = [
	...
	{ key: "descricao", header: "Descrição", render: (p) => p.descricao ?? "—" },
	{
		...
		render: (p) => (
			<span
				className={p.quantidade === 0 ? "text-destructive font-medium" : ""}
			>
				{p.quantidade}
			</span>
		),
							...
							{caixaAberto && sessao ? (
								<>
									<p className="text-sm font-medium text-green-700 dark:text-green-400">
										Caixa aberto às {fmtHora(sessao.abertoEm)} por{" "}
										{sessao.abertoPor}
									</p>
								</>
							) : (
					...
					<div className="flex items-center gap-2">
						{caixaAberto && sessao ? (
							<>
								<Badge
									variant="outline"
									className="text-green-700 border-green-300 dark:text-green-400"
								>
									Aberto
								</Badge>
								{canRegistrarVenda && (
				...
				<div className="flex items-center gap-2">
					...
					{canRegistrarVenda && (
						<Button
							...
							title={
								!caixaAberto ? "Abra o caixa para registrar vendas" : undefined
							}
						>
