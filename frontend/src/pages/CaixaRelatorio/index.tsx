...
export default function CaixaRelatorioPage() {
	...
	const baixarPdf = () => {
		...
		autoTable(doc, {
			startY: 63,
			head: [["Origem", "Total"]],
			body: relatorio.entradasPorOrigem.map((e) => [
				ORIGEM_LABELS[e.origem],
				fmtCurrency(e.total),
			]),
		});
		...
		autoTable(doc, {
			...
			head: [["Destino", "Total"]],
			body: relatorio.saidasPorDestino.map((s) => [
				DESTINO_LABELS[s.destino],
				fmtCurrency(s.total),
			]),
		});
		...
		autoTable(doc, {
			...
			body: relatorio.familiasBeneficiadas.map((f) => [
				f.familia,
				fmtCurrency(f.total),
			]),
		});
