...
export default function BazarRelatorioPage() {
  ...
  const [vendasPagination, setVendasPagination] = useState({ page: 1, pageSize: 15, totalCount: 0 })
  ...
  const loadVendas = useCallback(
    async (page: number, inicio: string, fim: string) => {
      try {
        ...
        setVendasPagination((prev) => ({ ...prev, page, totalCount: result.totalCount }))
      } catch {
    ...
    const todasVendas =
      vendasPagination.totalCount > 0
        ? await APIService.getRequest<PagedResponse<VendaBazar>>({
            url: '/bazar/vendas',
            params: { page: 1, pageSize: vendasPagination.totalCount, dataInicio, dataFim },
          }).then((r) => r.items)
        : []
  ...
  const columns: Column<VendaBazar>[] = [
    {
      ...
      header: 'Data / Hora',
      render: (v) => (
        <span className={`text-sm tabular-nums ${v.cancelado ? 'line-through text-muted-foreground' : ''}`}>
          {fmtDateTime(v.dataVenda)}
        </span>
      ),
    },
    {
      ...
      render: (v) => (
        <div className={v.cancelado ? 'text-muted-foreground' : ''}>
          ...
          {v.compradorCpf && <p className="text-xs text-muted-foreground">{v.compradorCpf}</p>}
          {v.compradorIdentificacaoAlternativa && (
            <p className="text-xs text-muted-foreground">ID: {v.compradorIdentificacaoAlternativa}</p>
          )}
        </div>
      ),
    },
    {
      ...
      render: (v) => (
        <div>
          <p className={`text-sm ${v.cancelado ? 'text-muted-foreground' : ''}`}>{v.registradoPor}</p>
          {v.cancelado && v.canceladoPor && (
            <p className="text-xs text-muted-foreground">Cancelado: {v.canceladoPor}</p>
          )}
          {v.cancelado && v.motivoCancelamento && (
            <p className="text-xs text-muted-foreground italic">"{v.motivoCancelamento}"</p>
          )}
        </div>
      ),
    },
    {
      ...
      header: 'Peças',
      render: (v) => (
        <div className={`space-y-0.5 ${v.cancelado ? 'text-muted-foreground line-through' : ''}`}>
          {v.itens.map((item, i) => (
            <p key={i} className="text-sm">
              {item.pecaCategoria} × {item.quantidade}{' '}
              <span className="text-muted-foreground">({fmtCurrency(item.valorUnitario)})</span>
            </p>
          ))}
        </div>
      ),
    },
    {
      ...
      render: (v) =>
        v.cancelado ? (
          <Badge variant="outline" className="text-xs text-muted-foreground">Cancelada</Badge>
        ) : (
    ...
    },
    {
      ...
      header: 'Total',
      render: (v) => (
        <span className={v.cancelado ? 'line-through text-muted-foreground' : 'font-semibold text-green-600'}>
          {fmtCurrency(v.valorTotal)}
        </span>
      ),
    },
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Resumo e histórico de vendas por período</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-xl border bg-card p-4 space-y-1">
              <p className="text-xs text-muted-foreground uppercase tracking-wide">Peças Vendidas</p>
              <p className="text-2xl font-semibold">{relatorio.totalPecasVendidas}</p>
            </div>
            <div className="rounded-xl border bg-card p-4 space-y-1">
              <p className="text-xs text-muted-foreground uppercase tracking-wide">Total Arrecadado</p>
          ...
          <div className="rounded-xl border bg-card overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr>
                  <th className="px-5 py-3 text-left font-semibold text-muted-foreground">Categoria</th>
                  <th className="px-5 py-3 text-right font-semibold text-muted-foreground">Qtd</th>
