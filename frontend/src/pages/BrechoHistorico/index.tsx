...
export default function BrechoHistoricoPage() {
  ...
  const baixarPdf = async () => {
    ...
    try {
      ...
      doc.setFontSize(10)
      doc.text(`${paroquiaAtual.label} — Sessão: ${fmtDateTime(sessao.abertoEm)}`, 14, 25)
          ...
          >
            <div className="space-y-0.5">
              <p className="font-medium">
                {sessao.aberto ? 'Sessão em andamento' : 'Última sessão encerrada'}
              </p>
              <p className="text-xs text-muted-foreground">
                Aberta em {fmtDateTime(sessao.abertoEm)} por {sessao.abertoPor}
                {sessao.fechadoEm && ` · Fechada em ${fmtDateTime(sessao.fechadoEm)} por ${sessao.fechadoPor}`}
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="text-right">
                ...
                <p className="font-semibold text-green-600">{fmtCurrency(totalVendas)}</p>
              </div>
            </div>
          </div>
