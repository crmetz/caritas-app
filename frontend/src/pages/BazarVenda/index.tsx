...
export default function BazarVendaPage() {
	...
	const nomeUsuario = session
		? `${session.nome} ${session.sobrenome}`.trim()
		: "";
	...
	const disponivel = (pecaId: number) =>
		pecas.find((p) => p.id === pecaId)?.quantidade ?? 0;
	...
	return (
		<div className="space-y-6 max-w-3xl">
			<div className="flex items-center gap-4">
				<Button variant="ghost" size="icon" onClick={() => navigate("/bazar")}>
					<ArrowLeft className="h-4 w-4" />
				</Button>
				<div>
					<h1 className="text-xl font-semibold">Registrar Venda — Bazar</h1>
								...
								<div className="grid grid-cols-[1fr_5rem_8rem_2.5rem] gap-3 items-end">
									<div className="space-y-1">
										...
										<Select
											...
											onValueChange={(v) =>
												updateItem(index, "pecaId", Number(v))
											}
										>
											<SelectTrigger>
				...
				<div className="grid grid-cols-2 gap-4">
					<div className="col-span-2 space-y-1">
						...
						<Input
							...
							onChange={(e) =>
								setComprador((prev) => ({ ...prev, nome: e.target.value }))
							}
						/>
					</div>
					<div className="space-y-1">
					...
					<div className="space-y-1">
						...
						<Input
							...
							onChange={(e) =>
								setComprador((prev) => ({
									...prev,
									identificacaoAlternativa: e.target.value,
								}))
							}
						/>
					</div>
					<div className="space-y-1">
						...
						>
							...
							<SelectContent>
								{(
									Object.entries(FORMA_PAGAMENTO_LABELS) as [
										FormaPagamento,
										string,
									][]
								).map(([k, v]) => (
			...
			<div className="flex justify-end gap-3">
				<Button variant="outline" onClick={() => navigate("/bazar")}>
					Cancelar
				</Button>
				<Button
					onClick={handleSubmit}
					disabled={loading || itensComErro.length > 0 || compradorInvalido}
				>
