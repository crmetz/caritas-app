...
export default function BazarHistoricoPage() {
  ...
  const totalVendas = data.filter((v) => !v.cancelado).reduce((s, v) => s + v.valorTotal, 0)
  ...
  const columns: Column<VendaBazar>[] = [
    ...
    },
    {
      ...
      render: (v) => {
        if (v.cancelado) {
          return <span className="text-xs text-muted-foreground italic">Cancelada</span>
        }
        return (
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-semibold">Bazar — Histórico de Vendas</h1>
            <p className="text-sm text-muted-foreground">
              {pagination.totalCount} {pagination.totalCount === 1 ? 'venda registrada' : 'vendas registradas'} · Total ativo:{' '}
              <span className="font-medium text-green-600">{fmtCurrency(totalVendas)}</span>
            </p>
          </div>
        </div>
        <Button
      ...
      <CancelarVendaModal ref={cancelarRef} onSuccess={() => load(pagination.page)} />
    </div>
  )
}
